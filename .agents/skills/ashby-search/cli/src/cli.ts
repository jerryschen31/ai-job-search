#!/usr/bin/env bun
// Self-contained CLI for searching jobs across companies hosted on Ashby, via
// Ashby's public posting API. No authentication, no API key, zero runtime
// dependencies.
//
// Ashby's API returns a company's entire board in one response and has no
// keyword parameter, so `search` fans out over a curated org list
// (src/companies.ts) and filters client-side.
...
const HELP = `ashby-cli — search jobs across companies hosted on Ashby
...
NOTES
  - Ashby has no cross-company search, so a bare \`search\` queries every org in
    src/companies.ts in parallel. Narrow with --company/--sector to cut volume.
  - Some boards are large (OpenAI's is ~12MB) because Ashby returns full
    descriptions inline; requests carry a 30s timeout for that reason.
  - Every result's \`url\` is the public jobs.ashbyhq.com posting link.

EXAMPLES
  bun run src/cli.ts search -q "machine learning" --sector biotech --format table
  bun run src/cli.ts search -q "engineer" -c insitro -c benchling --format table
  bun run src/cli.ts search -q "research" --remote --jobage 30 --format table
