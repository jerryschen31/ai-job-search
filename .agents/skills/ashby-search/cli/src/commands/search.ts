...
async function fetchBoard(
  ...
  try {
    const data = await jsonFetch<{ jobs?: AshbyJob[] }>(
      `${API_BASE}/${slug}?includeCompensation=true`,
    )
    if (!data?.jobs) return { jobs: [], error: "not found" }
    // isListed === false means the posting is unlisted/private on the board.
    return {
      jobs: data.jobs
        .filter((j) => j.isListed !== false)
        .map((j) => normalizeJob(j, slug, name)),
      error: null,
    }
  } catch (e) {
...
function renderTable(jobs: JobResult[]): string {
  ...
  const header =
    "TITLE".padEnd(42) + " " + "COMPANY".padEnd(18) + " " + "LOCATION".padEnd(24) + " DATE"
