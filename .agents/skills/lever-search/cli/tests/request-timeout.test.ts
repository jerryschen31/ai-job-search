...
describe("jsonFetch", () => {
  test("passes an AbortSignal timeout to fetch", async () => {
    ...
    await jsonFetch("https://api.lever.co/v0/postings/x");
    expect(init?.signal).toBeInstanceOf(AbortSignal);
  });

  test("returns null on 404 rather than throwing", async () => {
    globalThis.fetch = (async () => new Response("", { status: 404 })) as typeof fetch;
    expect(await jsonFetch("https://api.lever.co/v0/postings/x")).toBeNull();
  });

  test("throws on a non-retryable error status", async () => {
    globalThis.fetch = (async () => new Response("", { status: 403 })) as typeof fetch;
    await expect(jsonFetch("https://api.lever.co/v0/postings/x")).rejects.toThrow();
  });
});
