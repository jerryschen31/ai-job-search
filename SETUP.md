...
The job portal CLIs (LinkedIn, Built In, freehire, plus the ATS skills for Greenhouse/Ashby/Lever/Workday and the Levels.fyi and aggregator skills) are written in TypeScript and run with Bun.
...
You can generate a search skill for your local job board with `/add-portal` — it scaffolds the same CLI structure for any public portal and test-runs a live query before registering. See the "Job search tools" section in the README.
